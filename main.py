...
if data[0] == "class":
    ...
    for subject in classes[selected_class]:
        keyboard.append([
            InlineKeyboardButton(
                subject,
                callback_data=f"subject|{selected_class}|{subject}"
            )
        ])

    await query.edit_message_text(
        f"📚 اختر المادة في {selected_class}",
        reply_markup=InlineKeyboardMarkup(keyboard)
    )
    ...
    for t in types:
        keyboard.append([
            InlineKeyboardButton(
                t,
                callback_data=f"file|{selected_class}|{subject}|{t}"
            )
        ])

    await query.edit_message_text(
        f"📖 اختر نوع الملف لمادة {subject}",
        reply_markup=InlineKeyboardMarkup(keyboard)
    )
